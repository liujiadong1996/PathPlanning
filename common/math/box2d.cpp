...
Box2d::Box2d(const Vec2d &center, const double heading, const double length,
  ...
  if (length_ <= -kMathEpsilon) {
    std::cout << "[WARN]: length_ is smaller than kMathEpsilon!!" << std::endl;
  }
  if (width_ <= -kMathEpsilon) {
    std::cout << "[WARN]: width_ is smaller than kMathEpsilon!!" << std::endl;
  }
  InitCorners();
}
...
double Box2d::DistanceTo(const LineSegment2d &line_segment) const {
    ...
    case 2:
      return (x1 > x2) ? PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                       ...
                       : PtSegDistance(box_x, box_y, x1, y1, x2, y2,
                                       line_segment.length());
    case -1:
      return utils::CrossProd({x1, y1}, {x2, y2}, {box_x, -box_y}) >= 0.0
                 ? 0.0
                 : PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                                 line_segment.length());
    case -4:
      return utils::CrossProd({x1, y1}, {x2, y2}, {box_x, -box_y}) <= 0.0
                 ? PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                                 line_segment.length())
                 : (utils::CrossProd({x1, y1}, {x2, y2}, {-box_x, box_y}) <= 0.0
                        ? 0.0
                        : PtSegDistance(-box_x, box_y, x1, y1, x2, y2,
                                        line_segment.length()));
    }
  ...
  std::cout << "unimplemented state: " << gx1 << " " << gy1 << " " << gx2 << " "
            << gy2 << std::endl;
  return 0.0;
}
...
void Box2d::RotateFromCenter(const double rotate_angle) {
  heading_ = utils::NormalizeAngle(heading_ + rotate_angle);
...
std::string Box2d::DebugString() const {
  return "box2d ( center = " + center_.DebugString() +
         "  heading = " + std::to_string(static_cast<int>(heading_)) +
         "  length = " + std::to_string(static_cast<int>(length_)) +
         "  width = " + std::to_string(static_cast<int>(width_)) + " )";
}
