...
double LineSegment2d::DistanceTo(const Vec2d &point,
  ...
  if (nearest_pt == nullptr) {
    std::cout << "[ERROR]: nearest_pt is nullptr!!" << std::endl;
...
double LineSegment2d::DistanceSquareTo(const Vec2d &point) const {
  ...
  const double proj = x0 * unit_direction_.x() + y0 * unit_direction_.y();
  if (proj <= 0.0) {
    return utils::Square(x0) + utils::Square(y0);
  }
  if (proj >= length_) {
    return point.DistanceSquareTo(end_);
  }
  return utils::Square(x0 * unit_direction_.y() - y0 * unit_direction_.x());
}
...
bool LineSegment2d::IsPointIn(const Vec2d &point) const {
  ...
  // TODO(liujiadong)
  const double prod = utils::CrossProd(point, start_, end_);
...
bool LineSegment2d::GetIntersect(const LineSegment2d &other_segment,
  ...
  if (point == nullptr) {
    std::cout << "[ERROR]: point is nullptr!!" << std::endl;
    return false;
  }
  ...
  const double cc1 = utils::CrossProd(start_, end_, other_segment.start());
  const double cc2 = utils::CrossProd(start_, end_, other_segment.end());
  if (cc1 * cc2 >= -kMathEpsilon) {
    return false;
  }
  const double cc3 =
      utils::CrossProd(other_segment.start(), other_segment.end(), start_);
  const double cc4 =
      utils::CrossProd(other_segment.start(), other_segment.end(), end_);
...
double LineSegment2d::GetPerpendicularFoot(const Vec2d &point,
  ...
  if (foot_point == nullptr) {
    std::cout << "foot_point is nullptr!!" << std::endl;
    return point.DistanceTo(start_);
  }
  if (length_ <= kMathEpsilon) {
    *foot_point = start_;
