...
bool FindLaneSegment(const MapPathPoint &p1, const MapPathPoint &p2,
  ...
  for (const auto &wp1 : p1.lane_waypoints()) {
    for (const auto &wp2 : p2.lane_waypoints()) {
      if (wp1.lane->id() == wp2.lane->id() && wp1.s < wp2.s) {
...
std::vector<LaneSegment> Path::GetLaneSegments(const double start_s,
...
void Path::InitWidth() {
  ...
  //     lane_left_width_.push_back(3.5 / 2.0);
  //     lane_right_width_.push_back(3.5 / 2.0);

  //     road_left_width_.push_back(3.5 / 2.0);
  //     road_right_width_.push_back(3.5 / 2.0);
  //     std::cout << "[WARN]: path point:" << point.DebugString()
  //               << " has invalid width." << std::endl;
  //   } else {
  //     const LaneWaypoint waypoint = point.lane_waypoints()[0];
  //     if (waypoint.lane == NULL) {
  //       return;
  //     }
  ...
  //     &lane_right_width); lane_left_width_.push_back(lane_left_width -
  //     waypoint.l); lane_right_width_.push_back(lane_right_width +
  //     waypoint.l);
...
void Path::InitPointIndex() {
  ...
  //   s += kSampleDistance;
  // }
  // CHECK_EQ(last_point_index_.size(),
  // static_cast<size_t>(num_sample_points_));
}
...
void Path::InitOverlaps() {
  ...
  // &yield_sign_overlaps_); GetAllOverlaps(std::bind(&LaneInfo::stop_signs,
  // _1), &stop_sign_overlaps_); GetAllOverlaps(std::bind(&LaneInfo::crosswalks,
  // _1), &crosswalk_overlaps_); GetAllOverlaps(std::bind(&LaneInfo::junctions,
  // _1), &junction_overlaps_);
  // GetAllOverlaps(std::bind(&LaneInfo::pnc_junctions, _1),
  //                &pnc_junction_overlaps_);
  // GetAllOverlaps(std::bind(&LaneInfo::clear_areas, _1),
  // &clear_area_overlaps_); GetAllOverlaps(std::bind(&LaneInfo::speed_bumps,
  // _1), &speed_bump_overlaps_);
...
// std::vector<hdmap::LaneSegment>
// Path::GetLaneSegments(const double start_s, const double end_s) const {
...
bool Path::OverlapWith(const common::math::Box2d &box, double width) const {
  ...
  const double radius_sqr =
      utils::Sqr(box.diagonal() / 2.0 + width) + kMathEpsilon;
  for (const auto &segment : segments_) {
