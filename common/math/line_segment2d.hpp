...
class LineSegment2d {
 public:
   ...
   * @param start The start point of hte line segment
   ...
   * @return The end point of hte line segment
   */
   ...
   * @return The square of length of the line segmetn
   */
  double length_sqr() const;

  /**
   * @brief Compute the shortest distance from a point on the line segment to a
   * point in 2D
  ...
  /**
   * @brief Compute the shortest distance from a point on the line segment to a
   * point in 2D and get the nearest point on the line segment.
   * @param point The point to compute the distance to.
   * @param nearest_pt The nearest point on the line segment to the input point
   * @return The shortest distance from poinst on the line segment to the input
   * point.
   */
  double DistanceTo(const Vec2d &point, Vec2d *const nearest_pt) const;

  /**
   * @brief Compute the square of the shortest distance from a point on the line
   * segment to a point in 2D
  ...
  /**
   * @brief Compute the square of the shortest distance from a point on the line
   * segemtn to a point in 2D and get the nearest point on the line segment
   * @param point The point to compute the squared of the distance to.
   * @param nearest_pt The nearest point on the line segmetn
   *        to the input point
   * @return The shortest distance from points on the line segmetn to the input
   * point.
   */
  double DistanceSquareTo(const Vec2d &point, Vec2d *const nearest_pt) const;

  /**
   * @brief Check if a point is with in the line segment
   * @param point The point to check if it is within the line segment
   * @return Wheter the line point is within the line segment or not.
   */
  bool IsPointIn(const Vec2d &point) const;

  /**
   * @brief Check if the line segment has an intersect with another line segment
   * in 2D
   * @param other_segment The line segment to check if it has an intersect
   * @return Whether the line segment has an intersect withe the input
   * ohter_segment
   */
  bool HasIntersect(const LineSegment2d &other_segment) const;

  /**
   * @brief Compute the intersect with another line segmetn in 2D if any
   * @param other_segment The line segment to compute the intersect
   * @param point the computed intersect between the line segment and the input
   * other_segment
   * @return Wheter the line segment has an intersect with the input
   * other_segment.
   */
   ...
   * line segment) to compute the projectrion onto the line segment.
   * @return The projection of the vector, which is from the start point of the
   * line segment to the input point, onto the unit direction
   */
  double ProjectOntoUnit(const Vec2d &point) const;

  /**
   * @brief Compute the cross product of a vector onto the line segment
   * @param poin The end of the vector (starting from the start point of the
   * line segment) to compute the cross product onto the line segment
   * @return The cross product of the unit direction and the vector, which is
   * from the start point of the line segment to the input point.
   */
  double ProductOntoUnit(const Vec2d &point) const;

  /**
   * @brief Compute perpendicular foot of a point in 2D on the straight line
