...
bool Polygon2d::IsPointIn(const Vec2d &point) const {
  ...
  for (int i = 0; i < num_points_; ++i) {
    if ((points_[i].y() > point.y()) != (points_[j].y() > point.y())) {
      const double side = utils::CrossProd(point, points_[i], points_[j]);
...
void Polygon2d::BuildFromPoints() {
  ...
  for (int i = 1; i < num_points_; ++i) {
    area_ += utils::CrossProd(points_[0], points_[i - 1], points_[i]);
  }
  if (area_ < 0) {
    area_ = -area_;
  ...
  if (area_ < kMathEpsilon) {
    std::cout << "[WARN]: area is too small !!" << std::endl;
  }
  ...
  is_convex_ = true;
  for (int i = 0; i < num_points_; ++i) {
    if (utils::CrossProd(points_[Prev(i)], points_[i], points_[Next(i)]) <=
        -kMathEpsilon) {
      is_convex_ = false;
      break;
    }
...
bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d> &points,
  ...
  if (polygon == nullptr) {
    std::cout << "[ERROR]: polygon is nullptr!!" << std::endl;
    return false;
  }
  ...
  for (int i = 0; i < n + n; ++i) {
    ...
    while (count > last_count &&
           utils::CrossProd(points[results[count - 2]],
...
bool Polygon2d::ClipConvexHull(const LineSegment2d &line_segment,
  ...
  for (size_t i = 0; i < n; ++i) {
    prod[i] = utils::CrossProd(line_segment.start(), line_segment.end(),
                               (*points)[i]);
...
bool Polygon2d::ComputeOverlap(const Polygon2d &other_polygon,
  ...
  if (overlap_polygon == nullptr) {
    std::cout << "[ERROR]: overlap_polygon is nullptr!!" << std::endl;
    return false;
  }
  if (is_convex_ == false || other_polygon.is_convex() == false) {
    return false;
  }
...
bool Polygon2d::GetOverlap(const LineSegment2d &line_segment,
  ...
  if (first == nullptr || last == nullptr) {
    std::cout << "[ERROR]: first or last is nullptr!!" << std::endl;
    return false;
  }
...
Polygon2d Polygon2d::ExpandByDistance(const double distance) const {
  ...
  for (int i = 0; i < num_points_; ++i) {
    ...
    const double diff = utils::WrapAngle(end_angle - start_angle);
